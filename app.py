...
def get_image_extension(content: bytes) -> str:
    try:
        with Image.open(tempfile.NamedTemporaryFile(suffix=".tmp", delete=False).name) as img:
...
System: * I apologize, but it seems the code was cut off. Here's the complete corrected code with the `/shopify-face-swap` endpoint and all improvements integrated. I'll include the missing part of the `face_swap` function and the rest of the application, ensuring all functionality is preserved and enhanced.

<xaiArtifact artifact_id="1422b3ad-b7b3-48b6-b627-c66a4bea4921" artifact_version_id="c7435e2c-5385-4938-b330-3022d208dd5d" title="app.py" contentType="text/python">
import os
import uuid
import hashlib
import tempfile
